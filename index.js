...
const start = (client = new Client()) => {
    console.log('[DEVELOPER AND REEDITER]', color('Red Emperor And WhyyNotMe', 'yellow'))
    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                // conditions if the group members are less than 20 then the bot will leave the group
                if (ids.length <= 20) {
                    client.sendText(id, 'Maaf Bot Ini Hanya Untuk Di Gunakan Pada 20 Grup Saja. Bye~').then(() => client.leaveGroup(id))
                } else {
